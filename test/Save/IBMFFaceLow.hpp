...
public:
  ...
  bool load(const MemoryPtr dataStart, const int dataLength, uint8_t chSet) {
    ...
    for (int i = 0; i < faceHeader->glyphCount; i++) {
      ...
      if (memoryPtr > memoryEnd) {
        LOGE("IBMFFace: End of memory (0x%p, 0x%p, %d) reached reading glyph #%d!!", dataStart,
             memoryEnd, dataLength, i);
        return false;
      }
  ...
  bool getGlyph(char32_t charCode, Glyph &appGlyph, bool loadBitmap, bool caching = true,
    ...
    if (charSet == LATIN_CHARACTER_SET) {
      ...
      if (accentIsPresent) {
        ...
        if (accentIdx == CODED_GRAVE_ACCENT) accentIdx = GRAVE_ACCENT;
        else if (accentIdx == CODED_APOSTROPHE) accentIdx = APOSTROPHE;
    ...
    if (((glyphCode & GLYPH_CODE_MASK) == NO_GLYPH_CODE) ||
      ...
      LOGW("No entry for char code %d (0x%x) [glyph code %d (0x%x)]", charCode, charCode, glyphCode,
           glyphCode);
      glyphCode = SPACE_CODE;
    }
    ...
    appGlyph.metrics.xoff                    = -(glyphInfo->horizontalOffset + glyphOffsets.x);
    appGlyph.metrics.yoff                    = -(glyphInfo->verticalOffset + glyphOffsets.y);
    appGlyph.metrics.advance                 = (glyphInfo->advance + 32) >> 6;
  ...
  void showGlyph(const Glyph &glyph, uint16_t glyphCode, char16_t charCode = ' ') const {
    if constexpr (DEBUG) {
      std::wcout << "Glyph Base Code: 0x" << std::hex << glyphCode << std::dec << "(" << charCode
                 << ")" << std::endl
      ...
      if (glyph.bitmap.pixels != nullptr) { showBitmap(glyph.bitmap); }
    }
  }
  ...
  void showGlyphInfo(uint16_t i, GlyphInfo &g) const {
    if constexpr (DEBUG) {
      std::cout << "  [" << i << "]: "
                ...
                << ", advance: " << +((float) g.advance / 64.0) << ", dynF: " << +g.rleMetrics.dynF
      ...
      for (i = 0; i < faceHeader->ligKernStepCount; i++) {
        ...
        if (entry->skip.whole > 128) {
          std::cout << "  [" << i << "]:  "
                    ...
                    << "Goto: " << +((entry->opCode.d.displHigh << 8) + entry->remainder.displLow);
        } else {
          std::cout << "  [" << i << "]:  "
                    << "Whole skip: " << +entry->skip.whole << ", "
  ...
  void showFace() const {
    if constexpr (DEBUG) {
      ...
      for (int i = 0; i < faceHeader->glyphCount; i++) { showGlyphInfo(i, *glyphs[i]); }

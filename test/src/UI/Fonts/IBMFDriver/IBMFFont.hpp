...
private:
    ...
    bool ligKern(UTF8Iterator &iter, char32_t &chr, FIX16 &kern) {

    }

    void ligKernStream(const std::string &line) {
        if (line.length() == 0) return;
        UTF8Iterator iter = line.begin();
        char32_t chr = *iter++;
        FIX16 kern;
        while (iter != line.end()) {
          while (ligKern(iter, chr, kern)) ;
        ...
        if (isInitialized()) {
            for (UTF8Iterator chrIter = buffer.begin(); chrIter != buffer.end(); chrIter++) {
                ...
                if (face->getGlyph(*chrIter, glyph, false)) {
                    LOGD("advance value: %f, yoff value: %d", glyph.metrics.advance,
                         glyph.metrics.yoff);

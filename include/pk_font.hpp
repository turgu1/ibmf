...
private:
  ...
  bool get_packed_number(uint32_t &val) {
    uint8_t  nyb;
    uint32_t i, j, k;
    ...
    while (true) {
      ...
      if (i == 0) {
        ...
        if (i == PK_REPEAT_COUNT) {
          if (!get_packed_number(glyph_info.repeat_count))
            return false;
        } else { // i == PK_REPEAT_ONCE
          glyph_info.repeat_count = 1;
        }
        std::cout << '[' << glyph_info.repeat_count << ']' << ' ';
      }
    }
    return true;
  }

  bool glyph_short_preamble() {
    uint32_t val24;
    uint8_t  val8;
  ...
  bool glyph_preamble(uint8_t byte) {
    ...
    glyph_info.preamble_kind         = byte & 0x07;
  ...
  bool preamble() {
    ...
    if (getnext8(i)) {
      ...
      if (!getnext32(design_size))
        return false;
      if (!getnext32(checksum))
        return false;
      if (!getnext32(hppp))
        return false;
      if (!getnext32(vppp))
  ...
  bool retrieve_bitmap() {
    ...
    raster_count      = glyph_info.raster_size;
    ...
    if (glyph_info.is_a_bitmap) {
      ...
      for (uint32_t row = 0; row < glyph_info.bitmap_height; row++, rowp += row_size) {
        for (uint32_t col = 0; col < glyph_info.bitmap_width; col++) {
          ...
          if (black)
            rowp[col >> 3] |= (0x80U >> (col & 0x07));
          count--;
        }

        // if (glyph_info.repeat_count != 0) std::cout << "Repeat count: " <<
        // glyph_info.repeat_count << std::endl;
    ...
    if (raster_count != 0) {
      std::cerr << "Bitmap decoding error!" << std::endl;
      return false;
    }
    return true;
  }

  int load_glyph_table() {
    for (uint16_t i = 0; i < MAX_GLYPH_COUNT; i++)
      glyph_table[i] = nullptr;
    glyph_count = 0;
      ...
      default: {
        ...
        if (!(result = glyph_preamble(byte))) {
          ...
          if (glyph_info.char_code < MAX_GLYPH_COUNT) {
            ...
            std::cerr << "Char code out of limits: " << std::hex << "0x" << glyph_info.char_code
                      << std::dec << std::endl;
          }
        }
      }
...
public:
  PKFont(uint8_t *memory_font, uint32_t size) : memory(memory_font), memory_length(size) {
    font_size                    = 12;
  ...
  bool            get_glyph(uint8_t glyph_code, Glyph &glyph, bool load_bitmap) {
  ...
  bool show_glyph(const Glyph &glyph) {
    std::cout << "Glyph Char Code: " << glyph.char_code << std::endl
              ...
              << std::endl
              << "  Escapement: [" << glyph.horizontal_escapement << ", "
              << glyph.vertical_escapement << ']' << std::endl
              << "  Bitmap available: " << ((glyph.bitmap == nullptr) ? "No" : "Yes") << std::endl
              << "  Run-length encoding: " << (glyph.is_a_bitmap ? "No" : "Yes") << std::endl;
    ...
    for (row = 0, row_ptr = glyph.bitmap; row < glyph.bitmap_height; row++, row_ptr += row_size) {
      std::cout << '|';
      for (col = 0; col < glyph.bitmap_width; col++) {

...
private:
  ...
  bool                         load(MemoryPtr font_data, uint32_t length) {
    ...
    uint32_t *bin_face_offsets = (uint32_t *)data;
                            ...
                            uint32_t *bin_face_end_offsets = new uint32_t[preamble->face_count];
                            for (int i = 0; i < preamble->face_count; i++) {
                              bin_face_end_offsets[i] = (i < (preamble->face_count - 1)) ? bin_face_offsets[i + 1] : length;
    }

                            faces = new IBMFFaceLow[preamble->face_count];
                            for (int i = 0; i < preamble->face_count; i++) {
                              if (!faces[i].load(font_data + bin_face_offsets[i], font_data + bin_face_end_offsets[i]))
        return false;
    }
...
public:
  ...
  inline IBMFFaceLowPtr get_face(int idx) {

...
#pragma pack(pop)
  ...
  void read_glyph(uint8_t pk_char_code, uint16_t ibmf_char_code) {
    ...
    if (pk.get_glyph(pk_char_code, pk_glyph, false)) {
      ...
      glyph_info->glyph.advance =
          tfm.to_fix16(tfm.to_double(tfm.get_advance(pk_char_code), 20) * factor, 6);
  ...
  void read2_glyph(uint8_t pk_char_code, uint16_t ibmf_char_code) {
    ...
    if (pk2.get_glyph(pk_char_code, pk_glyph, false)) {
      ...
      glyph_info->glyph.bitmap_height     = pk_glyph.bitmap_height;
      glyph_info->glyph.horizontal_offset = pk_glyph.horizontal_offset;
      glyph_info->glyph.vertical_offset   = pk_glyph.vertical_offset + 1;
      glyph_info->glyph.packet_length     = pk_glyph.raster_size;
      glyph_info->glyph.advance =
          tfm2.to_fix16(tfm2.to_double(tfm2.get_advance(pk_char_code), 20) * factor, 6);
      glyph_info->glyph.glyph_metric.dyn_f          = pk_glyph.dyn_f;
      glyph_info->glyph.glyph_metric.first_is_black = pk_glyph.first_nibble_is_black ? 1 : 0;
      glyph_info->glyph.glyph_metric.filler         = 0;
  ...
  int read_lig_kerns() {
    ...
    for (auto &g : glyphs) {
      // std::cout << "Now doing glyph " << glyph_idx << " (0x" << std::hex << glyph_idx << std::dec
      // << ")" << std::endl;
...
next:
            ...
            if (((next_char_idx = get_char_idx(lks->next_char)) != -1) &&
              ...
              // std::cout << "Old tfm idx " << tfm_idx << " Saved at new idx " << lig_kerns.size()
              // << "..." << std::endl;
            ...
            if (!lks->skip.s.stop) {
              if (kept)
                lks = new TFM::LigKernStep; // If kept, create a new step, else reuse the old one
              *lks = tfm.get_lig_kern_step(++tfm_idx);
              // std::cout << tfm_idx << " c..." << " stop:" << (lks->skip.s.stop ? "YES" : "no") <<
              // std::endl;
              goto next;
            ...
            if (tfm_idx >= tfm.get_lig_kern_step_count())
              std::cout << "=============> GOING BEYOND THE END OF THE ARRAY!!! <================="
                        << tfm_idx << std::endl;
  ...
  void show() {
    ...
    std::cout << "DPI: " << header.dpi << ", point size: " << +header.point_size
              ...
              << ", descender height: " << +header.descender_height << std::endl;

    std::cout << std::endl << "----------- Glyphs: ---------- " << sizeof(Glyph) << std::endl;
    for (auto &g : glyphs) {

      std::cout << "  [" << i << "]: "
                << "pixelsIndex : " << +g->pixelsIndex << ", width: " << +g->glyph.bitmap_width
                ...
                << ", Old Index: " << +g->old_lig_kern_idx << std::endl;
      i++;
    }
    ...
    i = 0;
    std::cout << std::endl
              << "----------- NEW Ligature / Kern programs: ---------- " << sizeof(NewLigKernStep)
              << std::endl;
